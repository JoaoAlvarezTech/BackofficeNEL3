...
import { ArrowLeft, Home, Calendar, Wallet, FileText, Menu, Clock, TrendingUp, DollarSign, FileText as FileTextIcon, CheckCircle, AlertCircle, History } from "lucide-react";

type AntecipacaoConcluida = {
  id: string;
  amount: number;
  title: string;
  category: string;
...
const antecipacoesConcluidas: AntecipacaoConcluida[] = [
  { 
    id: "1", 
    amount: 1250, 
    ...
    date: "15/09/2024",
    completedDate: "12/09/2024",
    status: 'concluida',
    receivedAmount: 1212.50
  },
  { 
    id: "2", 
    amount: 890, 
    ...
    date: "22/09/2024",
    completedDate: "18/09/2024",
    status: 'concluida',
    receivedAmount: 863.30
  },
  { 
    id: "3", 
    amount: 2100, 
    ...
    date: "29/09/2024",
    completedDate: "25/09/2024",
    status: 'concluida',
    receivedAmount: 2037.00
  },
  { 
    id: "4", 
    amount: 650, 
    ...
    date: "06/10/2024",
    completedDate: "02/10/2024",
    status: 'concluida',
    receivedAmount: 630.50
  },
];

const HistoricoAntecipacoes = () => {
  const { logout } = useAuth();
  const { getCompletedActivities } = useAnticipation();
  const navigate = useNavigate();

  const completedActivities = getCompletedActivities();
  const totalAmount = completedActivities.reduce((sum, antecipacao) => sum + antecipacao.amount, 0);
  const totalReceived = completedActivities.reduce((sum, antecipacao) => sum + (antecipacao.receivedAmount || 0), 0);
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="bg-background/95 backdrop-blur-sm border-b border-border/30 p-4 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-foreground text-base font-semibold block">Histórico de Antecipações</span>
            <span className="text-muted-foreground text-xs">Antecipações concluídas e pagas</span>
          </div>
        </div>
      ...
      <div className="px-4 py-6 pb-28">
        {/* Summary Card */}
        <div className="bg-gradient-to-r from-green-500 to-green-600 rounded-3xl p-6 mb-6 shadow-lg">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-white font-bold text-lg">Antecipações Concluídas</h2>
                <p className="text-white/80 text-sm">Total processado</p>
              </div>
            </div>
            <div className="text-right">
              <div className="text-white font-bold text-2xl">
                R$ {totalAmount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                             ...
                             <div className="text-white/80 text-sm">{completedActivities.length} antecipações</div>
            </div>
          </div>
          <div className="bg-white/10 rounded-2xl p-3">
            <div className="flex items-center gap-2 text-white/90 text-sm">
              <DollarSign className="w-4 h-4" />
              <span>Total recebido: R$ {totalReceived.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
                      {completedActivities.map((antecipacao) => (
            <div
              key={antecipacao.id}
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-muted-foreground text-sm">Concluída em: {antecipacao.completedDate}</p>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <div className="text-right">
                    <p className="text-green-600 font-semibold text-sm">
                      Recebido: R$ {antecipacao.receivedAmount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                    </p>
                    <p className="text-muted-foreground text-xs">
                      Taxa: R$ {(antecipacao.amount - antecipacao.receivedAmount).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="mt-6 bg-primary/5 border border-primary/20 rounded-xl p-4">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-sm font-medium text-card-foreground">Histórico completo</p>
              <p className="text-sm text-muted-foreground mt-1">
                Aqui você encontra todas as suas antecipações que foram processadas e pagas com sucesso. 
                O valor recebido já está disponível em sua conta.
              </p>
            </div>
          </div>
        </div>
...
export default HistoricoAntecipacoes;
